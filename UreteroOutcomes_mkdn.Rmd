...
filter(u,MainRow=="Y") %>% group_by(sex) %>% count()
...
t <- t %>% mutate(total_rbus=ifelse(is.na(fu_rbus_date),baseline_hydro_ureterocele,follow_up_hydro_cele))%>% select(date_initial_rbus,fu_rbus_date,first_surgery_date,baseline_hydro_ureterocele,other_moiety_hydro,follow_up_hydro_cele,follow_up_hydro_cele_2,record_id,total_rbus)
...
t$total_rbus <- as.factor(t$total_rbus)
t$MultiSurg <- as.factor(t$MultiSurg)
summary(glm(MultiSurg ~ total_rbus, data = t, family = "binomial"(link='logit')))
#summary(glm(MultiSurg ~ relevel(factor(total_rbus),ref='2'), data = t, family = "binomial"(link='logit')))
#summary(glm(MultiSurg ~ relevel(factor(total_rbus),ref='3'), data = t, family = "binomial"(link='logit')))
#summary(glm(MultiSurg ~ relevel(factor(total_rbus),ref='4'), data = t, family = "binomial"(link='logit')))
...
t$MultiSurg <- as.factor(t$MultiSurg)
summary(glm(MultiSurg ~ total_rbus, data = t, family = "binomial"(link='logit')))
summary(t$total_rbus)
exp(coef(glm(MultiSurg ~ total_rbus, data = t, family = "binomial"(link='logit')))) # odds ratio = e^estimate
exp(confint(glm(MultiSurg ~ total_rbus, data = t, family = "binomial"(link='logit')))) 
...
```{r De novo reflux}
# How many had de novo reflux after primary TUI
k <- s %>% filter(postreflux=="Y"&prereflux=="N")
nrow(k) # how many ONLY postop reflux (not prereflux too)

# of the 18 who had de novo reflux, ow many had ureterocele_pole == 1 (upper pole) vs == 2 (simplex)
nrow(filter(k,ureterocele_pole == 1)) # simplex
nrow(filter(k,ureterocele_pole == 2)) # upper pole

# for the 18 de novo reflux people with simplex ureteroceles, how many had vur_fu > 0 vs how many had contra_reflux > 0
p <- unique(filter(k,ureterocele_pole == 1)$record_id)
length(unique(filter(u,record_id %in% p & (vur_fu > 0 & vur_fu <6))$record_id))
length(unique(filter(u,record_id %in% p & (contra_reflux > 0 & contra_reflux<6))$record_id))

# for the 18 de novo reflux people with upper pole ureteroceles, how many have vur_fu > 0 vs how many have vur_fu_2 > 0 vs how many have contra_reflux > 0
p <- unique(filter(k,ureterocele_pole == 2)$record_id)
length(unique(filter(u,record_id %in% p & (vur_fu > 0 & vur_fu<6))$record_id)) # upper
length(unique(filter(u,record_id %in% p & (vur_fu_2 > 0 & vur_fu_2<6))$record_id)) # lower
length(unique(filter(u,record_id %in% p & (contra_reflux > 0 & contra_reflux<6))$record_id)) # contra
length(unique(c(unique(filter(u,record_id %in% p & (vur_fu > 0 & vur_fu<6))$record_id),unique(filter(u,record_id %in% p & (vur_fu_2 > 0 & vur_fu_2<6))$record_id),unique(filter(u,record_id %in% p & (contra_reflux > 0 & contra_reflux<6))$record_id))))

length(intersect(unique(filter(u,record_id %in% p & (vur_fu > 0 & vur_fu<6))$record_id),unique(filter(u,record_id %in% p & (vur_fu_2 > 0 & vur_fu_2<6))$record_id))) # upper and lower
length(intersect(unique(filter(u,record_id %in% p & (contra_reflux > 0 & contra_reflux<6))$record_id),unique(filter(u,record_id %in% p & (vur_fu_2 > 0 & vur_fu_2<6))$record_id))) # lower and contra
length(intersect(unique(filter(u,record_id %in% p & (contra_reflux > 0 & contra_reflux<6))$record_id),unique(filter(u,record_id %in% p & ( (vur_fu > 0 & vur_fu<6)))$record_id))) # upper and contra
```
